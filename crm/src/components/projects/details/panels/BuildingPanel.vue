	...
	<div class="flex flex-col h-full bg-[var(--pd-bg-surface)] overflow-hidden">
		...
		>
				...
				<Popover ref="infoPopover">
					<div class="p-4 w-72">
						...
						<div class="mb-4">
							...
							>
								<span
									class="w-5 h-5 rounded-full text-[var(--pd-primary)] flex items-center justify-center text-[10px]"
								>
			...
			<div class="flex items-center gap-3">
				...
				<button
					class="w-8 h-8 flex items-center justify-center rounded-full border border-[var(--pd-border-light)] bg-[var(--pd-bg-base)] text-[var(--pd-text-secondary)] cursor-pointer transition-all duration-200 hover:bg-[var(--pd-bg-surface-hover)] hover:text-[var(--pd-text-primary)] hover:-translate-y-0.5 hover:shadow-sm"
					@click="showNewBuildingDialog"
				>
				...
				<div
					...
					:class="{
						'bg-[var(--pd-bg-surface-hover)] border-[var(--pd-border-medium)]':
							selectedBuilding && selectedBuilding.name === building.name,
					}"
				...
				>
					<div class="absolute top-1 right-1 flex">
						...
						<span
							...
							:class="{
								' text-[var(--pd-info)]':
									!building.status || building.status === 'Off-plan',
								'text-[var(--pd-warning)]': building.status === 'Due for Handover',
								' text-[var(--pd-success)]':
									building.status === 'Handover Completed',
							}"
						>
						...
						<span
							...
							:class="{
								' text-[var(--pd-success)]':
									...
									building.availability === 'Available',
								' text-[var(--pd-error)]': building.availability === 'Sold',
								'text-[var(--pd-warning)]':
									building.availability === 'Available for Secondhand',
							}"
						>
...
onMounted(async () => {
	...
	// Check for building ID in the URL and select it
	if (route.params.buildingId) {
		const buildingId = route.params.buildingId

		// Find the building in our list and select it
		const foundBuilding = buildings.value.find((b) => b.name === buildingId)
		if (foundBuilding) {
			selectedBuilding.value = foundBuilding
		} else {
			// If not found in current list, fetch it directly
			try {
				const building = await buildingStore.fetchBuilding(buildingId)
				if (building) {
					selectedBuilding.value = building
				}
			} catch (error) {
				console.error('Error fetching building from URL:', error)
			}
		}
	}
})

// Also watch for route changes to update selected building
watch(
	() => route.params.buildingId,
	async (newBuildingId) => {
		if (newBuildingId) {
			// If the building is already in our list
			const foundBuilding = buildings.value.find((b) => b.name === newBuildingId)
			if (foundBuilding) {
				selectedBuilding.value = foundBuilding
			} else {
				// Otherwise fetch it
				try {
					const building = await buildingStore.fetchBuilding(newBuildingId)
					if (building) {
						selectedBuilding.value = building
					}
				} catch (error) {
			...
			// Clear selection if no building ID in URL
	...
	filteredBuildings.value = buildings.value.filter(
		(building) =>
			...
			(building.building_configuration &&
				building.building_configuration.toLowerCase().includes(query)) ||
