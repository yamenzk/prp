...
if (!currentContext.value.docname && store.user) {
	currentContext.value.docname = store.session.user
}
...
const { journalNotes, journalsByDate } = useJournals(noteStore)

// Setup search composable for filtering notes
const { filteredNotes: filteredStickyNotes, filterNotesBySearch: filterStickyNotesBySearch } =
	useSearch(allStickyNotes, hideCompleted)

const { filteredNotes: filteredRegularNotes, filterNotesBySearch: filterRegularNotesBySearch } =
...
const { autoRooms, popularTags, isActiveContext, getContextColor, getContextFeatherIcon } =
...
// Add a new variable to track internal updates
...
watch(
	...
	(newVal) => {
		...
		if (!newVal) {
			noteStore.saveViewState(
				activeView.value,
				composingNote.value,
				composingNote.value ? draftNote.value : null,
			)
		}
	},
)

// Update the watch for context changes
watch(
	() => props.initialContext,
	(newContext) => {
		if (isInternalUpdate.value) return // Skip if we're already updating internally
...
const navigateBack = () => {
	if (activeView.value === 'detail') {
		...
		activeView.value =
			currentNote.value.rel_doctype === currentContext.value.doctype &&
...
const switchToContext = (context) => {
	isInternalUpdate.value = true // Set flag to indicate internal update
	...
	// Reset flag after a short delay to allow for the prop update
	setTimeout(() => {
...
console.log('Notes:', store?.session)
</script>
